...
<template>
  <div class="relative">
    <Password panelClass="dark:bg-background-dark dark:text-white" :id="id" :name="name" v-model="value as string"
      ...
      @focus="handleFocus" @focusout="handleBlur" @input="handleFocus" @date-select="handleFocus"
      @clear-click="handleBlur" @hide="handleBlur" :pt="{
      ...
      <template #footer>
        ...
        <ul class="pl-2 ml-2 mt-0 list-disc" style="line-height: 1.5">
          ...
          <li>
            <T keyName="form.input.password.prompt.suggestion.minimum.characters" />
          </li>
        </ul>
      </template>
    </Password>
    <div class="h-3 text-left">
      <span class="ml-3 text-error dark:text-error-dark dark:font-bold text-left text-xs">{{ errorMessage }}</span>
    </div>
    <label :for="id" class="absolute text-sm top-0 left-0 pl-3 py-4 pointer-events-none transition-all duration-300"
      :class="{
