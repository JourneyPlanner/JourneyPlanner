<script setup lang="ts">
import { ErrorMessage } from "vee-validate";

const props = defineProps({
    id: { type: String, required: true },
    value: { type: String, default: "" },
    name: { type: String, required: true },
    translationKey: { type: String, default: "" },
    customClass: { type: String, default: "" },
    inputType: { type: String, default: "" },
    icon: { type: String, default: "" },
    disabled: { type: Boolean, default: false },
    iconPosIsLeft: { type: Boolean, default: false },
});

const refValue = ref(props.value);
</script>

<template>
    <div class="mb-0 flex flex-col sm:mb-2">
        <label
            v-if="translationKey"
            :for="id"
            class="text-sm font-medium md:text-base"
        >
            <T :key-name="translationKey" />
        </label>
        <textarea
            :id="id"
            v-model="refValue"
            type="text"
            :name="name"
            :disabled="disabled"
            class="block h-full w-full overflow-hidden overflow-ellipsis rounded-lg border-2 border-border bg-input px-2.5 pb-1 pt-1 font-nunito font-normal text-text focus:outline-none focus:ring-1 dark:bg-input-dark dark:text-input"
        />
        <ErrorMessage
            :name="name"
            class="text-sm text-error dark:font-bold dark:text-error-dark"
        />
    </div>
</template>
